...
//DELETE
    ...
    $('#render').on('click','.del-butt',function(e){
        e.preventDefault();
        $('#agree').data('id',this.id);
        $("#slikja").show();
        $("#modal1").show();
       // setTimeout(function(){
            $('#agree').focus();
...
//Hér bý ég til note
    $("#addToNote").on('click', function () {
        var from = $("#input").val().trim();
        if(!from){
            // from er tómur strengur, null eða undefined (falsy).
            return;
        }
        var to = $("#to").val().trim();
        ...
        if(!category) {
            $("#category").focus();
            Materialize.toast('You need to fill category', 1000);
            return;
        }
        $.ajax({
            ...
            success: function(json) {
                ...
                console.log(json);
                renderText();
                Materialize.toast('Glossed', 1000);
                $('#input').focus();
                $('#input').select();
            }
        });
